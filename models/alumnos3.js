...
DB3.list = function(data, callback){
	DB3.alumnos.find({$or: [{name: data.filtro},{apellido: data.filtro},{carrera: data.filtro},{rut: data.filtro}]}).toArray(function(e,res){
		if(e){
			callback(e)
		}else{
			callback(null, res)
...
DB3.list2 = function(data, callback){
	DB3.alumnos.find({}).toArray(function(e,res){
		if(e){
			callback(e)
		}else{
			callback(null, res)
